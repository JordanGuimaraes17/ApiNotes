...
class NotesController {
  async create(request, response) {
    ...
    const [note_id] = await knex('notes').insert({
      // here, it will get the note ID to link with links and tags
      title,
      description,
      user_id
    })
    ...
    const linksInsert = links.map(link => {
      // create the link/url with the note id already created
      return {
        note_id,
  ...
  async show(request, response) {
    // to show one specific note according to the id typed by user
  ...
  async index(request, response) {
    ...
    if (tags) {
      ...
      notes = await knex('tags')
        ...
        .whereIn('tags.name', filterTags) // whereIn is going to search the tags typed in the tags recorded
